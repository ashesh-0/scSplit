...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        if self.opt["phase"] == "train":
            ...
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                self.optG, "max", patience=opt["train"]["lr_scheduler"]["patience"], factor=0.2, min_lr=1e-5
            )
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                ...
                if len(self.data["input"].shape) == 1:
                    ...
                    inp1, inp2 = self.netG.get_xt_clean(
                        ch1, ch2, torch.Tensor([0.5] * len(ch1)).to(ch1.device), update=True
                    )
                else:
                    ch1 = self.data["target"]
                    ch2 = self.data["input"]
                    inp1 = self.netG.get_xt_clean(
                        ch1, ch2, torch.Tensor([0.999] * len(ch1)).to(ch1.device), update=True
                    )
    ...
    def save_best_network(self, epoch, iter_step, metric_val):
        ...
        best_path = os.path.join(
            self.opt["path"]["checkpoint"], "best_gen_I{}_E{}_gen.pth.pth".format(iter_step, epoch)
        )
