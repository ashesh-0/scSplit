...
def get_datasets(opt, tiled_pred=False, eval_datasplit_type="val"):
    ...
    if data_type == "RRW":
        ...
        val_data_location = DataLocation(
            directory=(opt["datasets"]["val"]["datapath"]), datasplit_type=eval_datasplit_type
        )
        ...
        if not tiled_pred:
            ...
            tile_manager = get_tile_manager(
                data_shape, (1, patch_size // 2, patch_size // 2), (1, patch_size, patch_size)
            )
        ...
        val_kwargs = {}
        if data_type == "Hagen":
            train_data_location = DataLocation(
                channelwise_fpath=(
                    opt["datasets"]["train"]["datapath"]["ch0"],
                    opt["datasets"]["train"]["datapath"]["ch1"],
                )
            )
            val_data_location = DataLocation(
                channelwise_fpath=(opt["datasets"]["val"]["datapath"]["ch0"], opt["datasets"]["val"]["datapath"]["ch1"])
            )
        elif data_type in [
            "cifar10",
            "HT_LIF24",
            ...
            extra_kwargs["input_channel_idx"] = (
                opt["datasets"]["input_channel_idx"] if "input_channel_idx" in opt["datasets"] else None
            )
...
def get_xt_normalizer(train_set, train_opt, num_bins=100, num_steps=10000, dummy=False):
    xt_normalizer1 = NormalizerXT(num_bins=num_bins, device="cuda")
    xt_normalizer2 = NormalizerXT(num_bins=num_bins, device="cuda")
    if dummy:
        print("--------Dummy Normalizer Activated--------")
        return None, None  # xt_normalizer1, xt_normalizer2
...
def get_xt_normalizer_restoration(train_set, train_opt, num_bins=100, num_steps=10000, dummy=False):
    ...
    for _ in bar:
        ...
        for data in train_loader:
            ...
            for i in range(t_float_arr.shape[1]):
                inp = ch0 * (1 - t_float_arr[:, i].reshape(-1, 1, 1, 1)) + ch1 * (t_float_arr[:, i]).reshape(
                    -1, 1, 1, 1
                )
...
if __name__ == "__main__":
    ...
    if opt["datasets"]["train"]["name"] == "goPro2017dehazing":
        xt_normalizer1 = get_xt_normalizer_restoration(
            train_set, opt["datasets"]["train"], dummy=dummy_normalizer_flag, num_bins=100, num_steps=10000
        )
        xt_normalizer2 = None
    else:
                ...
                if current_step % opt["train"]["val_freq"] == 0:
                    ...
                    for _, val_data in enumerate(val_loader):
                        ...
                        for ch_idx in range(0, target.shape[0], ncols):
                            psnr_val = (
                                PSNR(target_img[ch_idx : ch_idx + ncols] * 1.0, pred_img[ch_idx : ch_idx + ncols] * 1.0)
                                .mean()
                                .item()
                            )
                        ...
                        Metrics.save_img(
                            pred_img, "{}/{}_{}_pred.png".format(result_path, current_step, idx), mode=mode
                        )
                    ...
                    logger_val.info(
                        "<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}".format(current_epoch, current_step, avg_psnr)
                    )
                    # tensorboard logger
